...
from config import DEVICE, TRAIN_BATCH_DOM, TRAIN_BATCH_BC, BC_TOP_PRESSURE

print("Testing boundary loss with 20MPa top pressure...")
solver = Solver()
gm = GeometryMaterial()
...
with torch.no_grad():
    ...
    # Top pressure boundary with target_disp_ratio = 0 and 1
    _, _, _, _, _, st, _, _, _, _, _, _, _ = solver.compute_physics(pts["top"], False, True)
    s33 = st[2]
    loss_pressure_0 = torch.mean((s33 + 0.0 * BC_TOP_PRESSURE)**2).item()
    loss_pressure_1 = torch.mean((s33 + 1.0 * BC_TOP_PRESSURE)**2).item()
    ...
    total = loss_bottom + loss_top_horiz + loss_pressure_0 + loss_side + loss_chem
    
    print(f"Loss bottom disp: {loss_bottom:.2e}")
    print(f"Loss top horiz disp: {loss_top_horiz:.2e}")
    print(f"Loss pressure (target 0): {loss_pressure_0:.2e}")
    print(f"Loss pressure (target full {BC_TOP_PRESSURE}MPa): {loss_pressure_1:.2e}")
    ...
    print(f"MODEL_OUT_SCALE_U = {solver.model.out_layer.weight.shape}")  # just to see
